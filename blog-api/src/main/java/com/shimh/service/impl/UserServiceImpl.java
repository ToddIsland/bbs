...
public class UserServiceImpl implements UserService {
    ...
    public Long saveUser(User user) {
        ...
        return userRepository.save(user).getId();//密码已经改变 是第一次加密之后的
    }
    ...
    @Override
    @Transactional
    public Long changeUserPassword(User user){
        PasswordHelper.encryptPassword(user);//对密码进行加密
        userRepository.modifyPasswordByUserAccount(user.getPassword(),user.getAccount());
        return user.getId();
    }
