<template>
  <div id="register" v-title data-title="注册 - Moyu">
    ...
    <div class="me-login-box me-login-box-radius">
      ...
      <div class="me-login-design">
        <p>Designed by
          <strong>
            <router-link to="/" class="me-login-design-color">BUAA</router-link>
          </strong>
        </p>
      </div>
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            that.$store.dispatch('register', that.userForm).then(() => {
              that.$message({message: '注册成功 快发帖子吧', type: 'success', showClose: true});//消息提示
              that.$router.push({path: '/'})//进入首页
            }).catch((error) => {
              if (error !== 'error') {
